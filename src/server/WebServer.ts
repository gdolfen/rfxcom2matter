...
export class WebServer {
  ...
  private setupRoutes(): void {
    ...
    // commissioned Matter clients (fabrics)
    this.app.get('/api/matter/fabrics', (_req, res) => {
      res.json(this.matter ? this.matter.getFabrics() : []);
    });
    this.app.delete('/api/matter/fabrics/:fabricIndex', async (req, res) => {
      const idx = Number(req.params.fabricIndex);
      if (!Number.isInteger(idx) || idx < 0) {
        res.status(400).json({ error: 'invalid fabricIndex' });
        return;
      }
      try {
        await this.matter?.removeFabric(idx);
        const fabrics = this.matter ? this.matter.getFabrics() : [];
        this.io.emit('matter-fabrics', fabrics);
        res.json({ ok: true, fabrics });
      } catch (err) {
        res.status(500).json({ error: (err as Error).message });
      }
    });
    this.app.post('/api/matter/open-commissioning', async (_req, res) => {
      try {
        await this.matter?.openCommissioning();
        res.json({ ok: true });
      } catch (err) {
        res.status(500).json({ error: (err as Error).message });
      }
  ...
  private setupSocket(): void {
    this.io.on('connection', (socket) => {
      ...
      socket.emit('matter-fabrics', this.matter ? this.matter.getFabrics() : []);
