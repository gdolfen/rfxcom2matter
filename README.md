...
> **Disclaimer:** Dieses Projekt wurde im Rahmen von **Vibecoding** erstellt –
> geschrieben mit [opencode](https://opencode.ai) unter Verwendung der Modelle
> **Hy3Free** und **DeepSeek V4 Flash Free**. Der Code wurde bisher **nicht
> durch einen Menschen geprüft oder kontrolliert**. Nutzung auf eigene Gefahr.
...
| Bereich | Gerät / Software |
|---|---|
| RFXCom | RFXtrx433 (USB) |
| Funk-Rollladenmotor | SIMU Hz mini receiver light, Simu T5 EHz Funk-Rollladenmotor |
| Matter-Controller | openHAB |
| MQTT | Home Assistant |
| TCP-Gateway | RFXmngr |
...
- [Deployment mit Docker Run](#deployment-mit-docker-run)
...
- [Entwicklung (lokaler Betrieb)](#entwicklung-lokaler-betrieb)
- [Release über GitHub Actions](#release-über-github-actions)
...
## Deployment mit Docker Run

```bash
docker pull ghcr.io/gdolfen/rfxcom2matter:1.2.3
docker run --device=/dev/ttyUSB0 -p 3000:3000 -p 5540:5540/udp \
  -v bridge-data:/app/data \
  ghcr.io/gdolfen/rfxcom2matter:1.2.3
```
...
| **Matter** | Pairing-Code + QR, Bridge-Einstellungen (Name, Port, Discriminator), Liste der angemeldeten Clients (entfernbar) |
...
3. Im Controller (Apple Home, Google Home, Home Assistant, openHAB) Gerät
   hinzufügen und den Code/die Kamera-Scans verwenden.
4. Die Bridge kündigt sich per mDNS an (`_matterc._udp.local`, UDP 5540).
5. Bereits gekoppelte Controller sind im Tab *Matter* unter
   „Angemeldete Clients" aufgelistet und können dort einzeln entfernt werden
   (sie müssen danach neu gekoppelt werden).
...
- **MatterBridge**: mDNS-Ankündigung + Pairing-Code/QR, verwaltet mehrere
  gekoppelte Controller (Fabrics) und deren Entfernung.
...
- `ghcr.io/gdolfen/rfxcom2matter:1.2.3` (Tag ohne führendes `v`)
- `ghcr.io/gdolfen/rfxcom2matter:latest`
